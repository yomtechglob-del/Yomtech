...
/* ─── 3D PENCIL INFOGRAPHIC COURSES (WATERMARK LOGO IN RIGHT BOTTOM CORNER) ─── */
        ...
        <div className="relative max-w-[90rem] mx-auto flex flex-col items-center">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 sm:gap-8 pt-6 sm:pt-8 border-t-2 border-slate-200/80 mt-2 w-full select-text cursor-text">
            {summaryColumns.map((col) => (
              <div 
                key={col.num}
                className={`p-7 sm:p-9 rounded-[2.5rem] ${col.bg} border-4 ${col.border} space-y-3 text-center shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-2 flex flex-col justify-between select-text cursor-text relative overflow-hidden group`}
              >
