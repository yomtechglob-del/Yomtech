...
import {
  Film, Video, Play, CheckCircle2, Clock, Calendar, ArrowRight, Phone,
  Sparkles, Layers, ShieldCheck, Eye, ExternalLink
} from 'lucide-react';
        ...
        <div className="max-w-[1720px] mx-auto px-6 sm:px-12 md:px-16 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center relative z-10">
          ...
          >
            <div className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-white/15 backdrop-blur-xl border border-white/30 text-white text-xs font-black shadow-lg">
              <Film size={14} className="text-cyan-300 animate-pulse" />
              <span className="text-cyan-200 font-bold uppercase tracking-widest text-[11px]">
                NATIONAL TECH DOCUMENTARIES
              </span>
            </div>
            ...
            <p className="text-cyan-50 text-base md:text-lg leading-relaxed font-normal max-w-2xl font-sans">
              High-definition technology documentaries capturing breakthrough innovations in artificial intelligence, satellite space science, e-government, and tech education across East Africa.
            </p>
            ...
            <div className="flex flex-wrap items-center gap-5 pt-3">
              ...
              >
                <span>Request Screening / Media License</span>
                <ArrowRight size={14} />
              </button>
            </div>
          </motion.div>
            ...
            <div className="w-full max-w-md bg-white/10 backdrop-blur-2xl border border-white/30 rounded-3xl p-6 shadow-2xl space-y-4">
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 rounded-2xl bg-white p-2 flex items-center justify-center">
                  <Film size={24} className="text-[#0284C7]" />
                </div>
                <div>
                  <h3 className="text-lg font-black text-white">4K Cinema Production</h3>
                  <p className="text-xs text-cyan-200">National Broadcast Media</p>
                </div>
              </div>
              <p className="text-xs text-white/80 leading-relaxed font-medium">
                Broadcasted across national television networks reaching millions of viewers to inspire youth in STEM careers.
              </p>
            </div>
          </motion.div>
        </div>
        ...
        <div className="max-w-[90rem] mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
          <div className="text-left space-y-3 max-w-4xl">
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-slate-900 font-display">
              Documentary <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#0284C7] via-cyan-600 to-indigo-600">Catalogue</span>
            </h2>
          </div>
              ...
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs font-bold text-slate-500 font-mono">
                    {doc.duration} · {doc.year}
                  </span>
                </div>

                <div>
                  <h3 className="text-xl font-extrabold text-slate-900 font-display">{doc.title}</h3>
                  <p className="text-xs font-bold text-[#0284C7] mt-0.5">Partner: {doc.partner}</p>
                </div>

                <p className="text-xs text-slate-600 font-medium leading-relaxed">{doc.desc}</p>

                <div className="space-y-1.5 pt-1 border-t border-slate-200">
                  <span className="text-[10px] font-black uppercase tracking-wider text-slate-400 block">Key Film Highlights:</span>
                ...
                >
                  ...
                  <span>Request Full Film Screening</span>
                </button>
              </motion.div>
            ))}
          </div>
